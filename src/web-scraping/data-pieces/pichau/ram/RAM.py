...
def RAM_Crawl():
    ...
    for i in range(10):
        ...
        if IPAddr == '192.168.2.38' or IPAddr == '192.168.2.75':  # Id verification
            ...
            # Crawling Products == Price
            product = driver.find_elements('class name', 'jss201')  # Possibles class name = jss191, jss69
            for i in product:
                if 'R$' in i.text:
                    pricesProducts.append(i.text)

            # Crawling Products == Installment Price
            product = driver.find_elements('class name', 'jss209')  # Possibles class name = jss199, jss77
            for i in product:
                if 'R$' in i.text:
                    installmentPriceProducts.append(i.text)
