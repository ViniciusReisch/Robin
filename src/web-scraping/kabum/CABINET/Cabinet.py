...
def Cabinet_Crawl():
    ...
    for i in range(1, 9):
        ...
        link = f'https://www.kabum.com.br/perifericos/gabinetes?page_number={i}&page_size=100&facet_filters=&sort=most_searched'
        driver.get(link)
        ...
        for i in links:
            if i.get_attribute('href') is None:
                continue
            if 'gabinete' in i.get_attribute('href') and 'produto' in i.get_attribute('href') \
                    or 'preto' in i.get_attribute('href') and 'produto' in i.get_attribute('href')\
                    or 'memory' in i.get_attribute('href') and 'produto' in i.get_attribute('href'):  # Only separate images with product in the name
    ...
    for i in range(len(pricesProducts)):
        ...
        dataDic = {'Store': 'Kabum', 'Name': namesProducts[i], 'Price': [pricesProducts[i], float(changeablePrices)],
                   ...
                   'Logo': 'https://static.pichau.com.br/logo-pichau-2021-dark.png', 'Type': 'Gabinete', 'Model': '', 'Color': ''}
